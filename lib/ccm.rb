...
module OpenSSL
  ...
  class CCM
    ...
    def initialize(cipher, key, mac_len)
      ...
      fail CCMError, 'invalid key length' unless
        key.b.length >= 16
      fail CCMError, 'invalid mac length' unless
        (4..16).step(2).include?(mac_len)
    ...
    def valid?(data, nonce, additional_data)
      fail CCMError, 'invalid nonce length' unless
        (7..13).include?(nonce.b.length)
      fail CCMError, 'invalid data length' unless
        data.b.length < 2**(8 * (15 - nonce.b.length))
      fail CCMError, 'invalid additional_data length' unless
        additional_data.b.length < 2**64
      true
    end
